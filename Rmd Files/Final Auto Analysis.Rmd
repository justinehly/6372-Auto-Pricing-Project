...
#Before we start to use model selection method, let's run linear regression to find out insignificant factor levels - going to include make to see how it affects other categorical variables


full.model<-lm(MSRP~.,data=autos) 
summary(full.model)

# Engine fuel - looks like anything including natural gas is insig
autos$Engine.Fuel.Type <- as.character(autos$Engine.Fuel.Type)

autos$Engine.Fuel.Type <- case_when(
  grepl("premium|recommended" , autos$Engine.Fuel.Type, ignore.case = TRUE) ~ "premium unleaded",
  grepl("unleaded" , autos$Engine.Fuel.Type, ignore.case = TRUE) ~ "regular unleaded",
  TRUE~autos$Engine.Fuel.Type)

autos$Engine.Fuel.Type <- as.factor(autos$Engine.Fuel.Type)

# Driven Wheels
autos$Driven_Wheels <- as.character(autos$Driven_Wheels)

autos$Driven_Wheels <- case_when(
  grepl("front|rear" , autos$Driven_Wheels, ignore.case = TRUE) ~ "2wd",
  TRUE~autos$Driven_Wheels)

autos$Driven_Wheels <- as.factor(autos$Driven_Wheels)

#noticed something about vans and car sizes - going to correct that real quick
autos$Vehicle.Size <- as.character(autos$Vehicle.Size)

autos$Vehicle.Size <- case_when(
  grepl("minivan", autos$Vehicle.Style, ignore.case = TRUE) ~ "Midsize",
  grepl("van", autos$Vehicle.Style, ignore.case = TRUE) ~ "Large",
  TRUE ~ autos$Vehicle.Size
)
...
autos$Vehicle.Style <- case_when(
  grepl("SUV", autos$Vehicle.Style, ignore.case = TRUE) ~ "SUV",
  grepl("2dr", autos$Vehicle.Style, ignore.case = TRUE) ~ "Coupe",
  grepl("4dr", autos$Vehicle.Style, ignore.case = TRUE) ~ "Sedan",
  grepl("van", autos$Vehicle.Style, ignore.case = TRUE) ~ "Van/ Minivan",
...
autos$Transmission.Type <- as.character(autos$Transmission.Type)
autos$Transmission.Type <- case_when(
  grepl("auto", autos$Transmission.Type, ignore.case = TRUE) ~ "Automatic",
  grepl("direct", autos$Transmission.Type, ignore.case = TRUE) ~ "Automatic",
  TRUE ~ "Manual"
)
...
ready_for_eda_autos <- autos #new stopping point in case we get lost in the next code chunk

```

```{r simple model by hand}

mod.simple <- lm(MSRP~., data=autos)
summary(mod.simple)

vif(aov(MSRP~., data=autos))
...
# start the code chunk fresh from the last code chunk
autos <- ready_for_eda_autos


EDA.model <- lm(MSRP~., data= autos)
summary(EDA.model)
...
cor_autos <- cor(autos[,c('MSRP','Engine.HP')],use='complete.obs')
