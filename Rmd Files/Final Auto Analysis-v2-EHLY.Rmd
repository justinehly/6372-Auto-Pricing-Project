...
plotDF <- data.frame(glmnet.pred, expm1(test$MSRP))
plotDF %>% ggplot(aes(x=expm1(glmnet.pred), y=test$MSRP)) +
...
```{r complex lasso}
set.seed(123)
model_afterlasso=lm(log1p(MSRP)~Transmission.Type+Driven_Wheels+Vehicle.Size+Hybrid
              +Hatchback + (Engine.HP)^2*(Engine.HP)^3, data=train)
...
RF.test<-postResample(pred=expm1(RF.pred), obs=test$MSRP)
RF.test
...
plotRF <- data.frame(RF.pred, test$MSRP)
plotRF %>% ggplot(aes(x=expm1(RF.pred), y=test$MSRP)) +
  geom_point(color="blue",show.legend = FALSE) +
  labs(title = "Random Forest Plot of Actual Test MSRP vs Predicted MSRP",
       x = "Predicted MSRP",
       y = "Test MSRP") +
  scale_y_continuous(label=dollar_format()) +
  scale_x_continuous(label=dollar_format()) +
...
#Validate
RF.valPred<-predict(RF.fit,validate)


RF.validate<-postResample(pred=expm1(RF.valPred),obs=validate$MSRP)
RF.validate

plotVal <- data.frame(RF.valPred, validate$MSRP)
plotVal %>% ggplot(aes(x=expm1(RF.valPred), y=validate$MSRP)) +
  geom_point(color="blue",show.legend = FALSE) +
  labs(title = "Random Forest Plot of Actual Validate MSRP vs Predicted Validate MSRP",
       x = "Predicted Validate MSRP",
       y = "Validate MSRP") +
  scale_y_continuous(label=dollar_format()) +
  scale_x_continuous(label=dollar_format()) +
  geom_abline(color="red") +
